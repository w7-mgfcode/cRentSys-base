...
?>
<DIV class="title">A megadott idõpontban a következõ jármûvek állnak rendelkezésre:</DIV>
...
?>
  <TR>
    <TD bgcolor="#e8d684" width="130">
      <A href="photos/<?php echo $row['kep']; ?>.jpg" target="_blank"><IMG src="photos/thumb/<?php echo $row['kep']; ?>.jpg" vspace="0" hspace="0" border="0"></A>
    </TD>
    <TD bgcolor="#e8d684" style="font-size: 11px;" valign="top">
      <DIV class="highlight"><?php echo $row['gyarto']." ".$row['tipus']; ?></DIV>
      <B>Extrák:</B> <?php echo $row['extra']; ?>
      <BR><B>Megjegyzés:</B> <?php echo $row['megjegy']; ?>
      <BR><B>Ár:</B> <?php echo $ara." Ft / ".$kulnap." napra"; ?>
      <FORM action="rent3.php" method="post">
        <INPUT name="kezdido" type="hidden" value="<?php echo $kezdstm; ?>">
        <INPUT name="vegeido" type="hidden" value="<?php echo $vegestm; ?>">
        <INPUT name="auto" type="hidden" value="<?php echo $auto; ?>">
        <INPUT name="autoar" type="hidden" value="<?php echo $ara; ?>">
        <INPUT type="submit" value="Kiválaszt" class="button">
      </FORM>
    </TD>
  </TR>
<?php
